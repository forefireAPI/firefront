...
/*
	void PYMNHInit(const double t){
		 MNHInit( t);
	}

	void PYCheckLayer(const char* lname){
		CheckLayer(lname);
	}

	void PYMNHStep(double dt){
		MNHStep(dt);
	}

	void PYMNHGoTo(double time){
		MNHGoTo(time);
	}

	void PYexecuteMNHCommand(const char* cmd){
		executeMNHCommand(cmd);
	}

	void PYFFPutString(const char* mname, char* str){
		FFPutString( mname, str);
	}

	void PYFFGetString(const char* mname, const char* str){
		FFGetString( mname, str);
	}

	void PYFFPutInt(const char* mname, int* n){
		FFPutInt( mname,  n);
	}

	void PYFFGetInt(const char* mname, int* n){
		FFGetInt(mname,  n);
	}


	void PYFFPutDouble(const char* mname, double* x){
		FFPutDouble( mname,  x);
	}

	void PYFFGetDouble(const char* mname, double* x){

		FFGetDouble( mname,  x);
	}

	void PYFFPutDoubleArray(const char* mname, double* x,	size_t sizein, size_t sizeout){

		FFPutDoubleArray(  mname,  x, sizein,  sizeout);
	}

	void PYFFGetDoubleArray(const char* mname, double t
			, double* x, size_t sizein, size_t sizeout){
		FFGetDoubleArray( mname,  t,  x,  sizein,  sizeout);
	}

	void PYFFDumpDoubleArray(size_t nmodel, size_t nip, const char* mname, double t
			, double* x, size_t sizein, size_t ni, size_t nj, size_t nk, size_t sizeout){
		FFDumpDoubleArray( nmodel,  nip,  mname,  t	,  x,  sizein,  ni,  nj,  nk,  sizeout);

	}

	void PYsaveNcRecord(int rec){
		saveNcRecord( rec);
	}
*/
