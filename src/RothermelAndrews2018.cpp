...
: PropagationModel(mindex, db) {
	...
	wv = registerProperty("normalWind_ms");
	slope = registerProperty("slope_deg");

	wo = registerProperty("fl1h_tac");
	fd = registerProperty("fd_ft");
	fpsa = registerProperty("SAVcar_ftinv");
...
double RothermelAndrews2018::getSpeed(double* valueOf){
	...
	double slope = Pi * valueOf[slope] / 180; // convert degrees to radians
	double wv = ms2ftmin * valueOf[wv]; // convert m/s to feat/min
	double pp = valueOf[pp];
	double mf = valueOf[mf];
	double fpsa = valueOf[fpsa];
	double fd = valueOf[fd];
	double wo = valueOf[wo];
	double h = valueOf[h];
	...
	if(wv < 0){
        ...
        double = 0.715 * exp(-3.59 * pow(10, -4) * fpsa);
        ...
        double FI = (384.0/fpsa)*RI*(R); // Uses Reaction Intensity in BTU / ft/ min
